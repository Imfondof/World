...
/**
 * DialogFragment源码，修改show中fragmnet.commit为fragmnet.commitAllowingStateLoss
 * 防止出现DialogFragment Can not perform this action after onSaveInstanceState 错误 造成程序崩溃
 * <p>
 * （区别：相对于commit，commitAllowingStateLoss不检查当前fragment的状态。虽然可能会丢失信息（onSaveInstanceState方法保存了fragment状态），但不会导致程序崩溃）
